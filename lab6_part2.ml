...
(* Alternative incompatible definitions that match the specification
   might reorder the node value and the two subtrees, for instance,

    type 'a bintree =
      | Leaf
      | Node of 'a bintree * 'a * 'a bintree ;;

   or use different value constructor names. *)
...
(* In this solution, we make use of an auxiliary function `min_option`
   (a monomorphic version of which you wrote in lab 4) to return the
   minimum of two `'a option` values. *)
...
(* A good further exercise is to define a `fold_tree` function and
   then reimplement `leaf_count`, `find`, `min_value`, and even
   `map_tree` in terms of `fold_tree`. *)
